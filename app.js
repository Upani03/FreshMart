...
function navigateTo(page) { window.location.href = page + '.php'; }
...
function renderCheckoutSummary() {
  ...
  if(!el) return;
  if(cart.length===0) { el.innerHTML='<p class="text-muted text-center py-3">Cart empty. <a href="products.php">Shop now</a></p>'; return; }
  var html='';
  for(var i=0;i<cart.length;i++) {
    var it=cart[i];
...
function placeOrder() {
  ...
  setTimeout(function(){ window.location.href='order-success.php'; },1400);
}
...
function renderOrders(filter) {
  ...
  if(orders.length===0) {
    container.innerHTML='<div class="orders-empty"><div style="font-size:4rem">&#128230;</div><h5 class="mt-3">No Orders Yet</h5><p class="text-muted">You have not placed any orders yet.</p><a href="products.php" class="btn btn-success rounded-pill px-4 mt-2">Start Shopping</a></div>';
    return;
  }
  var html='';
